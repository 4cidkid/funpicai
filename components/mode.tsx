...
export default function Mode({ prompts, mode, setMode,setSwitchImage }: ModeProps): JSX.Element {
    return (
        <div className='z-50 absolute left-2/4 -translate-x-2/4 top-12 flex items-center text-white '>
            ...
            <div className={`${prompts.length > 0 ? "opacity-0 pointer-events-none" : "opacity-100"} transition-opacity duration-300 relative bg-[#202123] flex items-center rounded-xl p-1 w-fit child:w-[150px] child:py-3 child:flex child:items-center child:justify-center`}>
                ...
                }} className={`${prompts.length === 0 ? "opacity-100" : "opacity-0 pointer-events-none"} ${mode ? "opacity-50" : "opacity-100"} rounded-xl cursor-pointer z-10 transition-opacity`}>
                    <span>
                ...
                <div onClick={() => {
                    setSwitchImage(true)
                    setMode(true);
                }} className={`${prompts.length === 0 ? "opacity-100" : "opacity-0 pointer-events-none"} ${mode ? "opacity-100" : "opacity-50"} rounded-xl cursor-pointer relative z-10 transition-opacity`}>
