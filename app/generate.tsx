...
export default function Generate({ prompts, setPrompts, mode, setMode, currentPrompt, setCurrentPrompt, setImageToEdit, canvasRef, imageToEdit,switchImage,setSwitchImage }: GenerateProps): JSX.Element {
    return (
        <div className={`${mode ? "md:w-[60%] " : "w-full"} max-md:w-full relative h-full flex justify-center`} style={{ transition: "width 700ms ease-in-out" }}>
            ...
            <Transition as={Fragment} show={mode && prompts.length > 0} enter="transition-opacity duration-500"
                ...
                leaveFrom="opacity-100"
                leaveTo="opacity-0">
                <span className={`md:w-[60%] max-md:w-full right-3 z-10 text-gray-300 text-sm  top-0 py-4 bg-[#343541]  fixed flex items-center justify-center`}>Edit</span>
            </Transition>

            <Mode setSwitchImage={setSwitchImage} prompts={prompts} mode={mode} setMode={setMode} />
            ...
            <div className=' z-1 w-full relative h-screen flex flex-col '>
                    ...
                    <div className='w-full py-3 flex-col'>
                        {
                            prompts.map((prompt, index) => {
                                return (
                                    <ChatLabel key={index} prompt={prompt} tilt={styles.tilt} />
                ...
                <PromptBar setSwitchImage={setSwitchImage} imageToEdit={imageToEdit} prompts={prompts} setPrompts={setPrompts} mode={mode} setMode={setMode} currentPrompt={currentPrompt} setCurrentPrompt={setCurrentPrompt} setImageToEdit={setImageToEdit} canvasRef={canvasRef} />
            </div>
        </div>
    )
}