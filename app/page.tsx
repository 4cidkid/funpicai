...
export default function Home(): JSX.Element {
  ...
  const [currentPrompt, setCurrentPrompt] = useState<{ prompt: string, active: boolean }>({
    prompt: "",
    active: true
  })
  const [promps, setPrompts] = useState<Array<{ promp: string, index: number, response: boolean, responseImage: string | null, loadingPropmt: boolean }>>([])

  const generateImage = async (prompt: string): Promise<void> => {
    setTimeout(() => {
      setPrompts((prev) => {
        const newArray = [...prev]
        newArray.push({ promp: "generating your " + prompt + "....", index: promps.length, response: true, responseImage: null, loadingPropmt: true })
        return newArray
      })
    },500)
    const response = await fetch("https://api.openai.com/v1/images/generations", {
      method: "POST",
      headers: {
        "Authorization": "Bearer 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",
    ...
    if (!response.ok) {
      if(data && data.error){
        if(data.error.code === "content_policy_violation"){
          toast.error("You prompt contains content that violates openAI content policy, please try again with a different prompt!")
        }
      }else{
    ...
    setPrompts((prev) => {
      ...
      arrayToModify[arrayToModify.length - 1].promp = arrayToModify[arrayToModify.length - 1].promp.replace("generating your ", "Here's your prompt: ")
      arrayToModify[arrayToModify.length - 1].responseImage = data?.data[0]?.url
    ...
    <section className='bg-[#fafafa] w-full h-screen'>
      ...
      <div className='bg-[#343541] flex items-center justify-center h-screen w-full overflow-hidden'>
        ...
        <div className={`${mode ? "w-[60%] " : "w-full"} relative h-full flex justify-center`} style={{ transition: "width 700ms ease-in-out" }}>
          {promps.length > 0 && <span id='generate' className='z-10 text-gray-300 text-sm  top-0 py-4 bg-[#343541] left-0  fixed flex items-center justify-center'>Generate</span>}
          <div className='z-50 absolute left-2/4 -translate-x-2/4 top-12 flex items-center text-white '>

            <div className={`${promps.length > 0 ? "opacity-0 pointer-events-none" : "opacity-100"} transition-opacity duration-300 relative bg-[#202123] flex items-center rounded-xl p-1 w-fit child:w-[150px] child:py-3 child:flex child:items-center child:justify-center`}>
              ...
              <div onClick={() => setMode(false)} className={`${promps.length === 0 ? "opacity-100" : "opacity-0 pointer-events-none"} ${mode ? "opacity-50" : "opacity-100"} rounded-xl cursor-pointer z-10 transition-opacity`}>
                <span>
                  <RiAiGenerate className='inline-block mr-2 text-green-400' />
                  Generate
                </span>
              </div>
              <div onClick={() => setMode(true)} className={`${promps.length === 0 ? "opacity-100" : "opacity-0 pointer-events-none"} ${mode ? "opacity-100" : "opacity-50"} rounded-xl cursor-pointer relative z-10 transition-opacity`}>
                <BiBrush className='inline-block mr-2 text-purple-400' />
                <span >Edit</span>
              </div>
            </div>
          </div>
          <h1 className={`${promps.length > 0 ? "opacity-0" : "opacity-100"} transition-opacity duration-300 font-bold text-[#565869] text-2xl absolute left-2/4 -translate-x-2/4 top-56`}>FunPic AI</h1>
                      ...
                      <div className={`${promps.response ? "bg-[#444654]" : ""} w-full`}>
                        <div key={index} className={`flex items-start gap-4 w-[500px] h-fit py-4 mx-auto`}>
                          ...
                          <div className='w-full'>
                            <p className={`${promps.loadingPropmt ? "tilt" : ""} break-words text-slate-200 text-sm leading-8`}>
                              {promps.promp}
                            </p>
                            {
              ...
              <div className='flex items-center justify-center gap-4 w-full'>
                ...
                <div className='relative w-full max-w-[500px]'>
                  <input onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                    setCurrentPrompt((prev) => ({ ...prev, prompt: e.target.value }))
                  ...
                  <PiPaperPlaneRightDuotone className={`${currentPrompt.active ? "" : "cursor-not-allowed pointer-events-none"} absolute right-2 top-2/4 -translate-y-2/4 text-gray-300 z-10 cursor-pointer`} onClick={() => {
                    if (currentPrompt.prompt.length === 0) return toast.error("Please enter a prompt!")
                    setPrompts([...promps, { promp: currentPrompt.prompt, index: promps.length, response: false, responseImage: null, loadingPropmt: false }])
                    setCurrentPrompt({
                      prompt: "",
                      active: false
                    })
                    generateImage(currentPrompt.prompt)
                  }} />
                </div>
              </div>
            </div>
